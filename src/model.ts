...
export const structured = async (params: StructuredParams) => {
  const { schema, provider, retryCount = 0, backoffFactor = 2 } = params;
  ...
  return await retry(
    async (_, attempts) => {
      ...
      if (typeof provider === "function") {
        processingResult = await processWithCustomHandler(
          { ...params, provider },
          prompt,
          last2Attempts
        );
        ...
        if (provider.url.includes("generativelanguage.googleapis.com")) {
          processingResult = await processWithGoogle(
            { ...params, provider },
            prompt,
            last2Attempts
          );
        } else if (provider.url.includes("api.anthropic.com")) {
          processingResult = await processWithAnthropic(
            { ...params, provider },
            prompt,
            last2Attempts
          );
          ...
          processingResult = await processWithOpenAI(
            { ...params, provider },
            prompt,
            previousAttempts
          );
      ...
      if (!validationResult.valid) {
        ...
        if (attempts - 1 >= retryCount) {
          ...
          throw new Error("Failed to validate result, retrying...");
        }
      }
